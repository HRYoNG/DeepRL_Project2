...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done, timestamp):
        ...
        if len(self.memory) > BATCH_SIZE and timestamp % UPDATE_CYCLE == 0:
            i = 0
            while i < SAMPLE_NUM:
    ...
    def learn(self, experiences, gamma):
        ...
        torch.nn.utils.clip_grad_norm_(self.critic_local.parameters(), 1) # use gradient clipping when training the critic network
        ...
        if  self.epsilon > EPS_END:
            self.epsilon *= (1.0 - EPS_DECAY_RATE)
        else:
